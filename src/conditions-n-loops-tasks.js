...
function canQueenCaptureKing(queen, king) {
  if (
    queen.x === king.x ||
    queen.y === king.y ||
    (queen.x / queen.y === 1 && king.x / king.y === 1) ||
    (queen.x + queen.y === 10 && king.x + king.y === 10)
  )
    return true;
  return false;
}
...
function convertToRomanNumerals(num) {
  ...
  const decimalsInitial = Math.floor(num / 10);
  const digitsInitial = num % 10;

  let decimalFinal = '';

  for (let i = 1; i <= +decimalsInitial; i += 1) {
    decimalFinal += obj[10];
  }

  return `${decimalFinal}${obj[digitsInitial]}`;
}
...
function rotateMatrix(matrix) {
  ...
  (function copyMatrixByReference() {
    for (let i = 0; i <= maxIndex; i += 1) {
      const row = matrix[i];
      for (let j = 0; j <= maxIndex; j += 1) {
        row[j] = newMatrix[i][j];
